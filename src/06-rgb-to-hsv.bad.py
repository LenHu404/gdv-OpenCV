...
print(
    "This is a RGB color detection demo. Use the keys to adjust the \
selection color in RGB space. Circle in bottom left."
)
print(
    "The masked image shows only the pixels with the given HSV color within \
a given range."
)
...
while True:
    ...
    if ret:
        ...
        img = cv2.putText(
            img, "R = " + str(red_channel), (width - 200, height - 75), font, font_size_smaller, red, thinner
        )
        img = cv2.putText(
            img, "G = " + str(green_channel), (width - 200, height - 50), font, font_size_smaller, green, thinner
        )
        img = cv2.putText(
            img, "B = " + str(blue_channel), (width - 200, height - 25), font, font_size_smaller, blue, thinner
        )
